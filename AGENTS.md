...
- **真回滚 / 恢复**：`node scripts/release.mjs <新版本号> --from-git <旧 ref>` ——
  ⚠️ **目前只实现了通道 A（热更新）**。通道 B 的对应能力（`release-apk.mjs --from-git`）**尚未实现** ——
  而且它与 web 不同：**Android 不允许 versionCode 更低的包覆盖安装**，所以"退回旧壳"只能是
  「旧壳代码 + **更高**的 versionCode」。在那之前，坏壳的唯一处置就是**尽快发一个修好的更高版本**。
