 ...
 *
 * `rollback.mjs`（2026-09-16）：它的 loadEnv 原本是另一套契约（返回对象 + throw），且脚本会写生产
 * （把版本置 enabled=false），当初刻意没迁移 —— 理由写在旧注释里：「迁移应当配一次真实下线演练一起做」。
 * 那条演练在补通道 B 下线能力时**真的做了**（下线 2.1.28 → 反向验证客户端视角 → restore →
 * 逐字段比对基线，11 个字段完全一致），所以迁移与演练一起落地。**「等条件具备再做」的待办，
 * 条件一到就该翻案** —— 这也是把它写下来的意义。
 *
 * 全部发布/校验类脚本现已统一走本模块，不再有各自的内联 `.env` 解析。
 */
