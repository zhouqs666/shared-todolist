...
- **每个批次/阶段收尾 → 把可讲的判断与踩坑追加到面试素材**（`~/Desktop/interview-prep` 的
  `16-CI_CD实战-面试知识点沉淀.md`，四段结构：是什么 / 面试怎么考 / 我的实证 / 面试话术）。
  ⚠️ **不要攒到"回头一起写"**：这条曾被拖到整段阶段 4 空着，等到补写时细节只能靠翻 commit message 回忆
  —— 素材的新鲜度就是它的价值。写完跑该工程的 `npm run build`（该目录有自己的 `AGENTS.md`，动手前先读）。
