...
- 🔴 **依赖「限时 UI」的用例，必须确认「操作延迟 < 该元素的生命周期」**（2026-09-17 复盘新增）
  一个元素如果在 N 秒后自己消失/失效，而驱动点一下要 M 秒，那么 M > N 时这条用例**永远点不到它** ——
  它会一直红（或一直绿，如果那个元素失效后仍然"可点"，见下）。判据两句话：
  ① 这个元素活多久？② 我点它要多久？两者必须量出来，别靠"应该来得及"。
  血泪（2026-09-17）：Appium 的「取消完成走撤销 Toast」用例两次 CI 连续红。量时间线才发现
  **本机 Appium 的 `elementClick` 每次约 10 秒**（UiAutomator2 点完要等应用 idle，而本应用有
  常驻无限动画 —— 心跳 / shimmer / 隐藏款光晕 —— 永远等不到 idle，于是每次耗到超时上限；
  同一次运行里其它 `elementClick` 也都是 10s 左右，可佐证），而撤销 Toast 只在屏上停留 2.5 秒。
  它以前**能过**，是因为 `.toast__action` 当时无条件 `pointer-events:auto` —— 收起后按钮仍是
  可点的透明热区，用例点的是那个残留热区，**靠一个缺陷蒙对了**。本次修掉该缺陷（误触会真的
  把已完成的待办改回去）后，点击如实落空：`elementClick` 报成功、库里 `completed` 纹丝不动
  —— 与本文件上方记录过的 #59 是同一类症状（命令成功、状态没变），只是成因不同。
  ⇒ 处置：① 用例改为不依赖窗口的等价入口（或 JS 触发），并在注释里写清为什么；
  ② **"修好一个缺陷后，把依赖它的用例一并找出来改"** —— 这类用例的红/绿本来就不代表功能对错。
  ⇒ 附带认知：`opacity: 0` 既**不参与命中测试**（所以"隐形可点"是真缺陷），
  也**不被 WebDriver 判定为不可见**（所以 `waitForDisplayed` 会对着一个隐形元素返回 true）。
  两件事叠加，正是这类用例能长期"蒙对"的土壤。

#### F2. 工作流 / CI 改动专项（2026-09-14 批次 A/B 复盘新增；批次 C 补充 2 条；批次 D 补充 4 条；2026-09-16 CI 卡死复盘补充 3 条；2026-09-17 CI 排队复盘补充 2 条）

- 触发条件：本次改动碰了 `.github/workflows/**` 或 `scripts/release*.mjs`（发布/CD 类脚本）。
  ℹ️ **2026-09-17 起 `e2e-app.yml` 已从 CI 删除**（模拟器套件移出，设备侧改由「发布时人工真机冒烟」
  承担，见 `AGENTS.md`「CI 分层」）。下面若干条以它为例的血泪记录**保留原样** —— 那些结论对
  **任何** workflow 都成立；要看当时的现场可 `git show <sha>:.github/workflows/e2e-app.yml`。

- 🔴 **加了 `paths` 之后，要问「这条流水线一次多少钱、多久」×「这个触发面会多频繁地命中它」**（2026-09-17）
  一个「有 `paths` 过滤」的工作流，不等于它的触发面是合理的。实测代价：`e2e-app.yml`（模拟器，
  一次约 **11 分钟**）原先含 `public/**` ⇒ **改一个颜色值就触发**；而它与夜间全量回归**共用并发组**
  且 `cancel-in-progress: false`（同一个测试库，必须串行）⇒ 触发即**占住队列、且不能被后来的取消**。
  实测排队：06:17 创建的 run 其 job 直到 06:23 才开始；06:27 那个等到 06:39。
  **判据三句**：① 它一次多久？② 它与谁共享并发组、能不能被取消？③ 它是不是 required？
  —— 若「很久 + 共享组不可取消 + 非 required」，那它**挡不住合并却能把队列堵住**，触发面必须收窄
  （或改成按需 `workflow_dispatch`）。血泪：前端小改动把整条流水线拖进十几分钟起步的排队，
  而收益只是"多一层设备验证"。
  **结局（2026-09-17）**：`e2e-app.yml` 先收窄触发面、再改为仅手动，最终**整体删除**
  —— 设备侧改由「发布时人工真机冒烟」承担（见 AGENTS.md「CI 分层」）。这说明这条判据的
  最终形态可能是「干脆别让它自动跑」，而不只是"收窄"。
- 🔴 **重试必须是「有意义的」，否则等于把失败时间乘二**（2026-09-17）
  给不稳的环节加重试是好习惯，但要问：**重试能不能改变结果？** 反例：`ci-run-with-retry.sh` 的第二次
  尝试是在**同一个模拟器会话**里重跑 —— 而设备已掉线时（`Could not find a connected Android device`），
  第二次只是把 11 分钟再烧一遍、结果一样。实测当天连续 4 次运行都这样白等一轮。
  ⇒ 做法：失败后**先分类**（设备还在吗？），基础设施故障用**专用退出码**告知调用方**跳过重试**；
  并且打印的话必须诚实——「测试没跑完」不能说成「已通过」。
  （实现见 `app-e2e/scripts/ci-run.sh`；随 `e2e-app.yml` 删除，该脚本**目前不再被 CI 调用**，
  仅为"将来恢复设备测试"保留，两处脚本头部都已标注。）
...
[2026-09-17] 已集齐的档位退出开奖抽选池（业主报的盲盒 bug）：🔴0 🟡2 → 通过
  · 业主报的现象原文：「查某一个类型的图鉴集齐了，添加待办依然继续开出同类型图鉴的 bug」。
    根因：`rollRarity()` 只看概率（15% 命中 + 6:3:1 分档），**从不看图鉴进度** —— 某一档 4 张
    集齐后仍会被开出：卡片显示该稀有度、撒花照放、对方端也收到揭晓，却一张贴纸都解锁不了。
    **"开出了"与"有东西可给"是两件事，旧实现只保证了前者。**
  · 数据安全（维度 A）：只读改动，零写操作 —— 修的是"少写"（白开的开奖本来就在空耗
    `unlockSticker` 的尝试）。E2E 全在测试库（三个 preflight 先过）。
  · 正确性（维度 B）—— 两个决定，各自的理由与代价：
    ① **档位判定用本地图鉴状态**（`availableRarities()`），不在开奖前查一次库：开奖在 `addTodo`
       的同步路径上，加一次网络往返等于把 15% 的惊喜绑到网络快慢上。代价是本地状态滞后时
       （冷启动 `listStickers` 未回 / 双端同刻开奖）单次仍可能白开 —— 已由「兜底路径顺手对齐」
       从"每次白开"降为"最多白开一次"，属**已知残留**，写进 PR 与规格（不是"应该没事"）。
    ② **12 张全齐后回落三档全池**（不是停发隐藏款）：集满的是玩得最深的两个人，
       把盲盒从他们的 App 里摘掉是"惩罚完成度"；只把兜底文案改成"图鉴 12 张已全部集齐，
       这张留作纪念 ✨"。
    ③ 权重在池内**重新归一**（相对比例不变：rare 集齐时 epic:legendary 仍是 3:1，不是各 50%），
       实测 76%（断言区间 0.68~0.82）；隐藏款总概率不变（实测 15.1% / 16.0%）。
  · 测试（维度 F）—— 原缺陷是"概率 × 进度"的组合，随机跑撞不到，故分两层钉住：
    · 构造图鉴状态驱动**真实** `rollRarity`：集齐档位 2000 次抽出 **0 次**；12 张全齐时断言
      **三档都出现过**（而不是"还能出隐藏款"—— 后者在回落逻辑写错时照样绿）
    · 用**真实图鉴状态**复现原场景（rare 真的 4/4 之后 1000 次开奖 0 次 rare）：构造状态只能证明
      "函数对"，真实状态才能证明"这一档在真实数据下确实退出了抽选池"
    · 12/12 文案由构造状态驱动 `onRollRarity`（该档已知 4/4 ⇒ 循环不执行 ⇒ 不写库），
      并把"**不写库**"也做成断言（`rare_keys()` 仍是 4 张）—— 用构造状态测写出路径最怕在真实库
      留垃圾，"没有写"要当断言而不是当假设
  · 🟡1（已记，未修）：`test_blindbox.py` 在**夜间全量回归**而非 PR 门禁（本仓 CI 分层的既定选择），
    所以这条回归在 PR 阶段跑不到 —— 本次是手工先跑的（58 项全过 / 80.6s）。要把这类**纯逻辑**
    纳入"快而稳"的门禁，正解是把加权抽选抽成零依赖模块 + Node 单测进 `ci.yml`；本次刻意不碰
    workflow（动 CI 按 F2 单列专项）。
  · 🟡2（既有问题，本次实测复现到；**与本改动无关**）：全量回归里 `test_trash` / `test_pin` 首跑
    FLAKY，同一失败族（卡片在超时窗口内没出现在列表里／撤销恢复没生效），重试通过。归因证据三条：
    ① `--no-retry` 连跑 3 轮共 6 次**全部直接通过**；② 同族超时截图在本机 **11:46–12:59 就存在**
    （当时 `blindbox.js` 当日未改动）；③ 代码路径不相交（本次改的是"开奖选哪一档"，失败点是
    "删除→撤销"与"卡片是否在 DOM 里"）。最可能成因：免费版测试库的瞬时失败 ——
    `db.restoreTodo` 抛错时产品**按设计**弹"撤销失败，请到回收站恢复"，界面表现与截图（空列表）一致。
    **未修**，另开专项；不混进本 PR。

[2026-09-18] 完成提示（toast）的几何与材质修复（业主报「完成待办的 toast 很难看」）：🔴0 🟡2（均已修）→ 通过
  · 起点是**实测**，不是调参：把「完成待办」的真实链路在 Pixel 7 / 390 / 360 三档视口跑出来量，
    四个根因全部带数字 —— ① `.toast` 是 `left:50%` + `right:auto` 的绝对定位，宽度按 shrink-to-fit
    算时可用宽只有 50vw，写着的 `max-width: 80vw` **从未生效**（三档实测宽度恰为 180/195/206px）；
    ② `align-items:center` 让勾图标在换行时掉到中间那行旁边（传说款图标 top=56/盒高 131）；
    ③ 白字压 `#e884a8` 只有 **2.53:1**（AA 要 4.5:1）、撤销按钮白字 2.12:1；
    ④ 全 App 唯一不带 `--safe-bottom` 的底部浮层（FAB/添加面板/图鉴/APK 面板都带）。
    **第五条是修完第一条才暴露的**：宽度放开后提示啃进 FAB（实测水平重叠 8~10px）⇒ 必须同批修。
  · 数据安全（维度 A）：零写操作 —— 纯 CSS + 文档 + 测试断言，无 SQL、无迁移、不碰生产数据。
    E2E 全程在测试库（三个 preflight 先过），跑完 `reset-test-db` 归零（实测 0 条残留）；
    另有一个 Node 回归（`test_sakura_colors`）按设计需生产服务在场，靠 `_lib-readonly-guard`
    在网络层阻断写请求 —— 输出自证「未向 Supabase 发起写请求」。
  · 🟡 **1｜我新加的断言第一版「不会咬人」，变异验证当场戳穿（本次最有价值的一条）**
    几何不变量断言加进 `test_undo_complete.py` 后跑绿；按惯例把 CSS `git checkout` 回退到修复前
    再跑，**4 条断言全绿** —— 等于没写。两个原因都属于「在错的前提下量」：
    ① E2E 跑在桌面视口（1280 宽），而缺陷是「宽度 = 视口的 50%」：640px 比文案还宽，换行不发生；
      产品是安卓 App（360~430）才会被压成 3 行。
    ② 普通款鼓励语是**随机抽**的：抽到「太棒了！」（4 字）时，坏掉的宽度下也照样单行。
    修法：几何探针显式切到 390×844 再量（量完还原）＋ 新增 H4 段直接喂**最长文案 + 撤销**走产品
    自己的 `showToast`（确定性，不靠抽签）。二次变异验证：修复前 **4 红**（普通 2 行 / 传说 5 行 /
    最长文案 3 行 / 基类 2 行），修复后 28 绿。
    ⇒ **教训**：断言写完必须让它对"已知坏掉的版本"红一次；否则它只证明「跑得通」，不证明「守得住」。
    （与维度 F 已记的"测试写了却抓不到 bug"同源，但这次是**新写的断言自己**踩的。）
  · 🟡 **2｜探针的对比度对渐变底色会误报 1:1**：`.toast--rarity` 的颜色在 `background-image` 里，
    `getComputedStyle().backgroundColor` 是 `transparent` ⇒ 合成后算成"白底压白字"。
    处置：识别到 `backgroundImage !== 'none'` 时**显式打印跳过并留痕**（该变体本来就是深底白字，
    归人眼与截图判定），不假装检过。
  · 💭 位置上移耦合了 `.fab` 的两个几何常量（24px 下边距 + 56px 直径），已在注释里写明来源；
    未引入 CSS 变量 —— 一次性耦合不值当新增 token，但将来改 FAB 尺寸要记得同步这一处。
  · 测试（维度 F）：三个 preflight ✅／`run-web-e2e.mjs` 5 用例全绿 **205.3s，0 flaky**
    （改完最后一版代码后重跑）／`test_undo_complete.py` 单跑 28/28 ✅／8 个 Node 回归 ✅
    （timeline_grouping 79、dora×2 42+36、rls/rpc 迁移、compress、update_count 16、pinch）／
    `test_sakura_colors` 49/49 ✅（新色值均在册，无色相漂移）／三项结构性检查 ✅。
  · 目检：三档视口 + 传说款最坏情况 + 基类长文案 + 删除撤销提示，六张截图逐一过目
    （探针与截图都在 /tmp，未入库）。
  · 文档同步（铁律四）：`PRODUCT-SPEC.md` §5.13 补「完成提示形态」一行、新增 §5.19 记录四个根因
    与修复、§9 登记 T-13（撤销按钮 3.96:1 差一口气，属该色系可取的最深档）。
  · **未验证（硬限制，明示）**：① **真机安全区**只在本机 Chromium 量到 `env()=0` ——
    手势条下压不压得住要等发布时真机冒烟（`insetsHandling:"css"` 的实际生效性同理）；
    ② `backdrop-filter: blur(12px)` 在低端安卓 WebView 的合成开销未测（面积小、单次 2.5~4s，
    预计可忽略，但没量过就是没量过）。
