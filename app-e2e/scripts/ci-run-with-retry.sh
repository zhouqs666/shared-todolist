#!/usr/bin/env bash
# 模拟器 E2E 的「失败重试一次」包装（原被 e2e-app.yml 调用）
#
# ⚠️ 2026-09-17 起 `e2e-app.yml` 已从 CI 删除 ⇒ 本脚本**当前不再被任何工作流调用**。
#    保留原因：它记录了「重试必须能改变结果」这条判据的实现（死在模拟器上就别重试）。
#    本地按需跑套件请直接 `cd app-e2e && npm test`。
#
