...
[2026-09-17] 去掉「置顶」章头左侧那枚图钉（v2.7.74）：🔴0 🟡1 💭0 → 通过
  · 业主诉求原文：「去掉待办页面，时光章节-置顶左侧的置顶图标」。
    ⚠️ 这句话有两处可指（章头的装饰图钉 / 卡片左端那枚 📌），执行前先按字面定位：
    "置顶**左侧**" = 挂在「置顶」二字左边的那个元素 —— 即 v2.7.72 给章头加的那枚
    `.tl-chap__pin`（`createChapter` 里只为 `tl-chap--pinned` 建的分支）。卡片那枚保留。
  · 改动（两处，纯删除）：`app.js` 删掉建图钉的分支（章头回到"文案 + 小计 + 分隔线"）；
    `style.css` 删掉 `.tl-chap--pinned .tl-chap__pin` 两条规则。**没有留下悬空 CSS**
    （`tl-chap__pin` 全仓 0 处引用，已 grep 核对）。卡片上那枚 `📌` 与长按菜单里的图钉**不动** ——
    章头说"这堆东西的位置"，卡片说"我这条被钉住了"，两者分工不同，删章头那枚才是去掉重复。
  · 回归断言（新）：`test_pin.py` 页内探针加 `icons`（章头这一行里除文案/小计/分隔线之外的
    图形成员数，只数直接子元素），断言**每个章头都是纯文字行**（置顶 / 要做的 / 时光章一致）。
    变异实验（`git stash` 回退 `app.js` + `style.css` 后跑同一份用例）：__pinned__ = 1 ⇒ 两条断言
    都红（28 通过 / 2 失败），恢复后 30/30 全绿 —— 断言确实咬得住，不是摆设。
  · 测试：三个 preflight ✅ ／ `run-web-e2e.mjs` 5 用例全绿（213.2s，0 flaky）／
    9 个 Node 回归 ✅（compress / pinch / sakura_colors / rls_migration / rpc_migration /
    dora×2 / update_count / timeline_grouping）三项结构性检查 ✅ ／
    目检截图（420×860@2x，测试库造数据后归零）：章头读作「置顶 · 1」，左侧无图标；卡片仍有 📌。
  · 🟡 **1｜测试库里一条 APP 通道残留会让本用例**假红**（本次实测踩到，与本次改动无关）**
    首次跑全量时 `test_pin.py` 连红两次：`keys(page) == ["__pinned__"]` 实得
    `["__pinned__", "__open__"]`。根因是测试库里躺着一条 `E2E-APP-标记完成-目标待办`
    （APP 通道残留，`reset-test-db.mjs` **刻意不删**它，以免动别的通道的数据），它未完成
    ⇒ 页面上多一个「要做的」章。**归因靠复现不靠推理**：把本次改动整体 stash 回退后，
    同一条断言照样红 ⇒ 与本次改动无关；该残留行随后已不在库里（何时被谁删的没查清，
    属测试环境噪声）。这正是本文件头部已写明的限制（"断言依赖页面上只有我造的这两条"）。
    **决定不放宽断言**：把它改成"包含 __pinned__ 即可"会连带放过"页面上凭空多出内容"这类真问题
    —— 宁可承受偶发假红并在报告里说明，也不牺牲这条不变量。
